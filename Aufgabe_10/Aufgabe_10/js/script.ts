...
var openCounterDOMElement: HTMLElement;
var doneCounterDOMElement: HTMLElement;
var counterDone: number;
var counterOpen: number;
var newTask: tasks;
...
window.addEventListener("load", function(): void {
    ...
    openCounterDOMElement = document.querySelector("#openCounter");
...
function drawListToDOM(): void {
    ...
    for (let index: number = 0; index < allTasks.length; index++) {
         ...
         */
        todo.innerHTML =  "<span class='check " + allTasks[index].todosChecked + "'><i class='fas fa-check'></i></span>"
                            + allTasks[index].todosText +
...
function updateCounter(): void {
    counterDOMElement.innerHTML = allTasks.length + " in total";
}

function updateOpenCounter(): void {
    counterOpen = 0;
    for(var i: number = 0; i<allTasks.length; i++){
        if(allTasks[i].todosChecked == false){
            counterOpen++;
        }
    }
    openCounterDOMElement.innerHTML = counterOpen + " open";
}

function updateDoneCounter(): void {
    counterDone = 0;
    for(var z: number = 0; z<allTasks.length; z++){
        if(allTasks[z].todosChecked == true){
            counterDone++;
        }
    }
...
function addTodo(): void {
    ...
    if (inputDOMElement.value != "") {
        ...
        newTask = {todosText: inputDOMElement.value, todosChecked: false};
...
function toggleCheckState(index: number): void {
     ...
     */
    allTasks[index].todosChecked = !allTasks[index].todosChecked;
    /**
     * Die zentrale Funktion, um die Liste des ToDo-Arrays in den DOM zu rendern
     * wird wieder getriggert
     */
...
function addByVoice(input: string){
    if (input != "") {
        newTask = {todosText: input, todosChecked: false};
        allTasks.unshift(newTask);
        inputDOMElement.value = "";
        drawListToDOM();
    }
...
function checkAllTasks(){
    for(var v: number = 0; v<allTasks.length; v++){
        allTasks[v].todosChecked = true;
    }
    drawListToDOM();
}
...
window.addEventListener("load", function(): void {
    ...
    artyom.addCommands({
        ...
        smart: true,
        action: function(i, wildcard): void {
            addByVoice(wildcard);
        }
    });

    artyom.addCommands({
        indexes: ["Markiere alle Aufgaben als erledigt"],
        smart: false,
        action: function(i): void {
            checkAllTasks();
        }
    });
